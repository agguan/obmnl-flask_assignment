...
def get_transactions():
    ...
    # Render the transactions.html template, passing both the transactions and the total balance
...
# Search operation
# Route to handle searching for transactions within a specified amount range
@app.route("/search", methods=["GET", "POST"])
def search_transactions():
    if request.method == "POST":
        # Retrieve min and max amounts from the form and convert to floats
        min_amount = float(request.form['min_amount'])
        max_amount = float(request.form['max_amount'])

        # Filter the transactions within the amount range
        filtered_transactions = [
            transaction for transaction in transactions
            if min_amount <= transaction['amount'] <= max_amount
        ]

        # Render the transactions.html template, passing the filtered transactions
        return render_template("transactions.html", transactions=filtered_transactions)

    # If GET request, render the search form
    return render_template("search.html")

# Calculate total balance operation
@app.route("/balance")
def total_balance():
    # Calculate total balance by summing all transaction amounts
    balance = sum(transaction['amount'] for transaction in transactions)
    
    # Return total balance as a formatted string
